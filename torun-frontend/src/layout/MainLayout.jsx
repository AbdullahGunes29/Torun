...
const MainLayout = () => {
  ...
  // SES OYNATMA FONKSİYONU TAMAMEN KALDIRILDI
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      // Eğer gelen yanıtta bir komut yapısı varsa işlem yap
      if (botResponse.includes('|||')) {
        ...
        if (cmd.command === "list") {
          ...
          // Güncelleme veya detay görme isteği geldiğinde
        ...
        // HERHANGİ BİR KOMUT YOKSA (ÖRN: SELAMLAŞMA) YAN PANELİ KAPAT/AÇMA
      ...
      setMessages(prev => [...prev, { id: Date.now() + 1, text: "Sistemde geçici bir sorun oluştu.", sender: 'bot' }]);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <>
              ...
              <div className="max-w-2xl mx-auto py-10">
                {messages.length === 0 && (
                  <div className="text-center py-20">
                    ...
                    <p className="text-xl text-gray-500 italic text-center">"Ürünlerimi listeler misin?" diyerek başlayabilirsiniz.</p>
                  </div>
                )}
            ...
            <form onSubmit={handleSendMessage} className="p-6 bg-[#131314]">
              <div className="max-w-2xl mx-auto relative">
                <input type="text" value={inputText} onChange={e => setInputText(e.target.value)} placeholder="Asistana sorun efendim..." className="w-full bg-[#1e1f20] border border-gray-800 rounded-full py-4 px-6 pr-16 outline-none focus:border-blue-500 transition-all shadow-2xl" />
  ...
  return (
    <div className="flex h-screen w-full bg-[#131314] text-[#e3e3e3] overflow-hidden">
      <Sidebar isOpen={isSidebarOpen} setIsOpen={setIsSidebarOpen} currentView={currentView} setCurrentView={handleNavigate} clearChat={handleNewChat} />
      <main className="flex-1 flex relative">
        {!isSidebarOpen && (
          <button onClick={() => setIsSidebarOpen(true)} className="absolute top-5 left-5 z-50 p-2 bg-[#1e1f20] hover:bg-gray-800 rounded-lg border border-gray-800 text-gray-400 transition-all">
